...
export async function createPedido(req, res) {
  ...
  try {
    const clienteRes = await fetch(`${process.env.CLIENTES_URL}/clientes/${cliente_id}`);
...
export async function getPedidoEnriquecido(req, res) {
  ...
  try {
    ...
    const clienteRes = await fetch(`${process.env.CLIENTES_URL}/clientes/${pedido.cliente_id}`);
